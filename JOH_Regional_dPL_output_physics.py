...
#Create the model, the model structure remains unchanged
...
#Read the data and standardize it, using the mean and standard deviation of the training period
...
model = create_model((10957, 32), (10957, 32),hodes=256, seed=200)#10957 is the length of the time series
model.summary()

representation_model = Model(inputs=model.inputs, outputs=model.get_layer('Regional_dPL_LSTM').output)


#Performing cycle tests
for j in range(len(testx_list)):

    dense_hydro_output = representation_model.predict([test_x, test_x1])#Get intermediate variables
